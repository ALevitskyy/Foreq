...
class DataManager:
    ...
    def get_norm_params(self,
                        currency,
                        horizon):
        table = self.raw_data[currency].resample(horizon)\
                      .aggregate({"C":"last","O":"first"}).dropna()
        candles = table["C"]-table["O"]
        self.norm_params[currency][horizon] = {"mean": np.mean(candles),
                                                "std": np.std(candles)}
    ...
    def binarize(self, value, currency, horizon):
        dummy = pd.get_dummies(pd.cut([value],
                              self.bins[currency][horizon]))
