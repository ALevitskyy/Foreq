...
class ResNet(nn.Module):
    def __init__(
        ...
        self.conv1 = nn.Conv1d(
            ...
            padding=3,
            bias=False,
        )  # 1st ARGUMENT - THE INPUT CHANNELLS
        ...
        self.fc = nn.Linear(512 * block.expansion, horizon * bins)
        self.horizon = horizon
        self.bins = bins
        self.softy = nn.Softmax(2)
    ...
    def forward(self, x):
        ...
        x = self.layer1(x)
        x = self.layer2(x)
        x = self.layer3(x)  # 256x16x16
        x = self.layer4(x)  # 512x8x8

        x = self.avgpool(x)  # 512x1x1
        x = x.reshape(x.size(0), -1)
        x = self.fc(x)
        x = x.view(-1, self.horizon, self.bins)
        x = self.softy(x)
        return x
