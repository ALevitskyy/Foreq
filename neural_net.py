...
def conv3x3(in_, out):
    return nn.Conv1d(in_, out, 3, padding=1)
...
class AlbuNet(nn.Module):
    def __init__(self, encoder,out_channels=9, num_classes=1, num_filters=32):
        super().__init__()
        self.num_classes = num_classes

        self.pool = nn.MaxPool1d(2) #2,2
        ...
        self.final = torch.nn.Conv1d(256, out_channels, 1) 
    def forward(self, x):
        conv1 = self.conv1(x)
...
def make_net():
    encoder = resnet18()
    return AlbuNet(encoder, out_channels = 3)